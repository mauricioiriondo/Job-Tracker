...
import { ExternalLink, Trash2, Pencil, DollarSign, GraduationCap } from "lucide-react";
...
function InterestIndicator({ level }: { level: string }) {
  switch (level) {
    ...
    case "Medium":
      return (
        <span className="inline-flex items-center gap-1 text-xs font-medium text-yellow-500 dark:text-yellow-400" data-testid="interest-medium">
          👍 Medium
        </span>
      );
    case "Low":
      return (
        <span className="inline-flex items-center gap-1 text-xs font-medium text-green-600 dark:text-green-400" data-testid="interest-low">
          🤷 Low
        </span>
      );
    default:
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {prospect.sponsorsVisa && (
            <span className="inline-flex items-center gap-1 text-xs font-medium text-blue-600 dark:text-blue-400" data-testid={`badge-sponsors-visa-${prospect.id}`}>
